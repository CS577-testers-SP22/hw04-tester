...
TIMEOUT_TEST = True
TIMEOUT_TEST_FILE = 'timeout.json'
...
def test8_timeout_tests():
    if not TIMEOUT_TEST:
        ...
        return
    try:
        with open(TIMEOUT_TEST_FILE, 'r') as f:
            timeout_tests = json.load(f)
    except FileNotFoundError as e:
        print(f'Skipping Timeout Tests as {TIMEOUT_TEST_FILE} could not be opened.')
        return
    
    print('Running Timeout Tests. These can take a very long time if your code is not efficient. Set TIMEOUT_TEST=False on line 39 if you don\'t want to test this.')
    run_tests(timeout_tests)
...
def get_versions():
    ...
    try:
        with urlopen('https://raw.githubusercontent.com/CS577-testers-SP22/hw04-tester/master/.version') as f:
            if f.status != 200:
                raise Exception
...
if __name__ == '__main__':
    ...
    if current < latest:
        ...
        print('You can download the latest version at https://github.com/CS577-testers-SP22/hw04-tester\n')
    print()
    main()
    print()
